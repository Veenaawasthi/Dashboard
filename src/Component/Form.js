...
const Form = ({ formData, onSubmit, onCancel }) => {
  ...
  useEffect(() => {
    if (formData) {
      setFormState(formData); // Sync form state with passed formData when editing
    }
  }, [formData]);
  ...
  const handleServiceChange = (dayIndex, serviceIndex, e) => {
    ...
    const updatedDays = [...formState.days];
    updatedDays[dayIndex].services[serviceIndex].name = value;
    setFormState((prevData) => ({
      ...prevData,
      days: updatedDays,
    }));
  ...
  const handleDurationChange = (dayIndex, serviceIndex, e) => {
    const { value } = e.target;
    const updatedDays = [...formState.days];
    updatedDays[dayIndex].services[serviceIndex].duration = value;
    setFormState((prevData) => ({
      ...prevData,
      days: updatedDays,
    }));
  ...
  const addQuotationSlabRow = () => {
    setQuotationSlabs([...quotationSlabs, { slab: '', maxPax: '', minPax: '', noOfFOC: '', ppCost: '' }]);
  };
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    onSubmit(formState); // Ensure onSubmit is a function
  };

  const serviceOptions = [
    { value: '', label: 'NRT/HND Airport Arrival Transfer' },
    { value: 'Option 1', label: 'Arrival Flight Details: TBA' },
    { value: 'Option 2', label: 'After disembarking, please follow the signs to the Immigration' },
    { value: 'Option 3', label: 'After clear3333ting immigration, collect your baggage & meet our representive' },
  ];

  return (
    <div className="container" style={{ maxWidth: '800px', margin: '0 auto', padding: '20px', backgroundColor: '#799678', border: '1px solid #ccc', borderRadius: '8px' }}>
      <h1 style={{ fontFamily: 'Arial, sans-serif', color: 'black' }}>ITINERARY FORM</h1>
      <form onSubmit={handleSubmit}>
        <div id="itineraryForm">
          <div style={{ backgroundColor: '#fff', padding: '20px', border: '1px solid #ddd', borderRadius: '8px' }}>
            <div style={{ display: 'flex', alignItems: 'center',backgroundColor:'lightblue' }}>
              <img src={process.env.PUBLIC_URL + '/Logo RD.jpg'}  className="company-logo" style={{ maxWidth: '200px', marginRight: '20px' }} /> 
        ...
        <h2 style={{ fontFamily: 'Arial, sans-serif', color: '#333', marginTop: '20px' }}>Itinerary Details</h2>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr>
                ...
                <th style={{ border: '1px solid #ddd', padding: '8px', textAlign: 'left' ,alignItems: 'center', textDecoration: 'none' }}><img src={process.env.PUBLIC_URL + '/city.png'} style={{ padding: '0.5px', width: '24px', height: '24px', backgroundColor:'blue' }}/>City</th>
                <th style={{ border: '1px solid #ddd', padding: '8px', textAlign: 'left', alignItems: 'center', textDecoration: 'none'  }}><img src={process.env.PUBLIC_URL + '/activity.png'} style={{ padding: '0.5px', width: '24px', height: '24px' }}/>Service</th>
                <th style={{ border: '1px solid #ddd', padding: '8px', textAlign: 'left', alignItems: 'center', textDecoration: 'none'  }}><img src={process.env.PUBLIC_URL + '/duration.png'} style={{ padding: '0.5px', width: '24px', height: '24px' }}/>Duration</th>
                <th style={{ border: '1px solid #ddd', padding: '8px', textAlign: 'left', alignItems: 'center', textDecoration: 'none'  }}>Mode</th>
                <th style={{ border: '1px solid #ddd', padding: '8px', textAlign: 'left', alignItems: 'center', textDecoration: 'none'  }}><img src={process.env.PUBLIC_URL + '/meal.png'} style={{ padding: '0.5px', width: '24px', height: '24px' }}/>Meal</th>
              </tr>
            </thead>
            <tbody>
              {formState.days.map((day, dayIndex) => (
                <tr key={dayIndex}>
                  ...
                  <td style={{ border: '1px solid #ddd', padding: '8px', textAlign: 'left' }}>
                    <input type="text" value={day.date} onChange={(e) => handleDayChange(dayIndex, e)} id="date" style={{ width: 'calc(100% - 10px)', padding: '8px', border: '1px solid #ccc', borderRadius: '4px', fontSize: '14px' }} />
                  </td>
                  <td style={{ border: '1px solid #ddd', padding: '8px', textAlign: 'left' }}>
                    <input type="text" value={day.city} onChange={(e) => handleDayChange(dayIndex, e)} id="city" style={{ width: 'calc(100% - 10px)', padding: '8px', border: '1px solid #ccc', borderRadius: '4px', fontSize: '14px' }} />
                  </td>
                  <td style={{ border: '1px solid #ddd', padding: '8px', textAlign: 'left' }}>
                    <select value={day.services[0].name} onChange={(e) => handleServiceChange(dayIndex, 0, e)} style={{ width: 'calc(100% - 10px)', padding: '8px', border: '1px solid #ccc', borderRadius: '4px', fontSize: '14px' }}>
                  ...
                  <td style={{ border: '1px solid #ddd', padding: '8px', textAlign: 'left' }}>
                    <input type="text" value={day.services[0].duration} onChange={(e) => handleDurationChange(dayIndex, 0, e)} style={{ width: 'calc(100% - 10px)', padding: '8px', border: '1px solid #ccc', borderRadius: '4px', fontSize: '14px' }} />
                  </td>
                  <td style={{ border: '1px solid #ddd', padding: '8px', textAlign: 'left' }}>
      ...
      <div className="form-table" style={{ marginTop: '10px' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead>
            <tr>
              <th style={{ border: '1px solid #ddd', padding: '8px', textAlign: 'left' }}>Slab</th>
              <th style={{ border: '1px solid #ddd', padding: '8px', textAlign: 'left' }}>Max Pax</th>
              <th style={{ border: '1px solid #ddd', padding: '8px', textAlign: 'left'}}>Min Pax</th>
              <th style={{ border: '1px solid #ddd', padding: '8px', textAlign: 'left' }}>No. of FOC</th>
              <th style={{ border: '1px solid #ddd', padding: '8px', textAlign: 'left' }}>PP Cost</th>
          ...
          <tbody>
            {quotationSlabs.map((slab, index) => (
              <tr key={index}>
                ...
                <td style={{ border: '1px solid #ddd', padding: '8px', textAlign: 'left' }}>
                  {quotationSlabs.length !== 1 && <button type="button" onClick={() => removeQuotationSlabRow(index)} style={{ backgroundColor: '#dc3545', color: '#fff', border: 'none', padding: '6px 12px', borderRadius: '4px', cursor: 'pointer', width: '100%' }}>Remove</button>}
                </td>
              </tr>
            ))}
          </tbody>
        ...
        <div style={{ marginTop: '20px' }}>
          ...
          <button type="button" onClick={onCancel} style={{ padding: '10px 20px', border: 'none', backgroundColor: '#dc3545', color: '#fff', borderRadius: '4px', cursor: 'pointer' }}>Cancel</button>
        </div>
      </form>
    </div>
  );
